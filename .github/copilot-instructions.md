...
2. Create a `.temp/plan-copilot.md` file with:
...
1. Inform the user that you've created `.temp/plan-copilot.md`
...
3. After completing each step, update the plan-copilot.md:
...
1. **Check for existing plan**: Look for `.temp/plan-copilot.md`
...
- **"resume"** or **"continue"**: Read `.temp/plan-copilot.md` and resume from last checkpoint
...
- ✅ Keep plan-copilot.md synchronized with actual progress
- ✅ Check for existing plans when starting new tasks
- ✅ If tries to run `npm run dev`, always try to kill existing process first

### MUST NOT DO:
- ❌ Never modify files before plan approval
- ❌ Never skip the planning phase
- ❌ Never assume approval without explicit confirmation
- ❌ Never delete plan-copilot.md (archive if needed: `plan_YYYYMMDD_HHMMSS.md`)
...
1. The plan-copilot.md preserves all progress
...
[Creates .temp/plan-copilot.md]

I've created a plan in .temp/plan-copilot.md with 7 steps:
